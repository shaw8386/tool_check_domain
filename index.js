...
const REQUEST_TIMEOUT_MS = parseInt(process.env.REQUEST_TIMEOUT_MS || "15000", 10);
const MAX_REDIRECT_FIX = parseInt(process.env.MAX_REDIRECT_FIX || "3", 10);
// Concurrency limit for parallel processing (number of rows processed simultaneously)
...
/**
 * Extract text content from HTML and return first N words
 * Excludes code, HTML tags, JavaScript, CSS, and other non-text content
 */
function extractTextFromHTML(html, maxWords = 500) {
  try {
    if (!html || typeof html !== "string") return "";
    
    const $ = cheerio.load(html);
    
    // Remove all script, style, and code-related elements
    $("script, style, noscript, iframe, embed, object, code, pre").remove();
    
    // Remove elements with common code-related classes/ids
    $("[class*='code'], [class*='Code'], [id*='code'], [id*='Code']").remove();
    $("[class*='script'], [class*='Script']").remove();
    $("[class*='syntax'], [class*='Syntax']").remove();
    ...
    // Get text content from body, prioritizing main content areas
    let text = "";
    ...
    // Remove code-like patterns (e.g., function(), var x =, etc.)
    text = text.replace(/[a-zA-Z_$][a-zA-Z0-9_$]*\s*[=:]\s*[^,\s;]+/g, "");
...
async function processRow(row) {
  ...
  for (let slot = 1; slot <= maxSlots; slot++) {
    ...
    try {
      ...
      // Always record the HTTP status code, even if it's not 200
      ...
      // FAIL: xử lí theo status (but we still have the status code recorded above)
      const fixPlan = getFixPlan({
        status: result.status,
        errorCode: null,
        currentUrl,
      ...
      // ghi log ra console để Railway log
      console.error(`[${getVietnamLogTime()}] [FAIL] domain=${domain} slot=${slot}/${maxSlots} url=${currentUrl} proxy=${proxyUrl ? "YES" : "NO"} err=${msg} lastStatus=${lastStatus || "none"}`);
    }
  }
  ...
  for (let i = 0; i < rows.length; i += CONCURRENCY_LIMIT) {
    ...
    const batchPromises = batch.map(async (row, batchIndex) => {
      ...
      try {
        ...
        console.error(`[${getVietnamLogTime()}] Error processing row ${globalIndex} (${row.Domain || "N/A"}):`, error.message);
        // Return a failed result
        return {
          Domain: row.Domain || "",
          Proxy_IP_PORT_USER_PASS: row.Proxy_IP_PORT_USER_PASS || "",
          ISP: row.ISP || "",
          DNS: row.DNS || "",
          Maxtime_try: row.Maxtime_try || "",
          Max_Slot_try: row.Max_Slot_try || "",
